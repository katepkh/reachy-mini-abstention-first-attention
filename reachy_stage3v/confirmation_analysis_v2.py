"""Held-out evaluation for the frozen Stage 3V V2 policy."""
...
from .confirmation_protocol_v2 import (
    CONFIRMATION_V2_STEPS,
    CONFIRMATION_V2_SUCCESS_CRITERIA,
    confirmation_v2_protocol_payload,
)
from .revised_policy import aggregate_revised_trials
from .revised_policy_v2 import FROZEN_REVISED_POLICY_V2, evaluate_revised_trial_v2


def confirmation_v2_quality_issues(step: Any, summary: dict[str, float | int]) -> tuple[str, ...]:
    limits = CONFIRMATION_V2_SUCCESS_CRITERIA["instrumentation_quality"]
    issues: list[str] = []
...
def evaluate_confirmation_v2_trial(step: Any, rows: list[dict[str, Any]]) -> dict[str, Any]:
    summary = summarise_rows(rows)
    revised = evaluate_revised_trial_v2(step, rows, FROZEN_REVISED_POLICY_V2)
    return {
        **revised,
...
def evaluate_confirmation_v2_files(csv_files: list[str]) -> dict[str, Any]:
    if len(csv_files) != len(CONFIRMATION_V2_STEPS):
        raise ValueError("A held-out Stage 3V V2 evaluation requires 18 accepted files.")
    trials: list[dict[str, Any]] = []
    modes: set[str] = set()
    for step, filename in zip(CONFIRMATION_V2_STEPS, csv_files):
        path = (DATA_DIR / filename).resolve()
        if path.parent != DATA_DIR or not path.is_file():
            raise ValueError(f"Held-out V2 file is missing: {filename}")
        rows = load_rows(path)
        issues = confirmation_v2_quality_issues(step, summarise_rows(rows))
        if issues:
            raise ValueError(f"Held-out V2 file fails instrumentation quality: {filename}: {'; '.join(issues)}")
        compliance_path = _compliance_path(path)
        if not compliance_path.is_file():
            raise ValueError(f"Held-out V2 file lacks procedural compliance review: {filename}")
        compliance = json.loads(compliance_path.read_text(encoding="utf-8"))
        if compliance.get("verdict") != "COMPLIANT":
            raise ValueError(f"Held-out V2 file is not procedurally compliant: {filename}")
        modes.add(str(compliance.get("data_mode") or "unknown"))
        trials.append({**evaluate_confirmation_v2_trial(step, rows), "file": filename})

    aggregate = aggregate_revised_trials(trials)
    return {
        "schema": "reachy-stage3v-held-out-confirmation-result-v2",
        "status": "PASSIVE_V2_HELD_OUT_VALIDATION_ONLY_NOT_APPROVED_FOR_ACTUATION",
        "protocol_fingerprint": confirmation_v2_protocol_payload()["fingerprint"],
        "policy_fingerprint": FROZEN_REVISED_POLICY_V2.payload()["fingerprint"],
        **aggregate,
        "trials": trials,
...
def write_confirmation_v2_results(csv_files: list[str]) -> dict[str, Any]:
    result = evaluate_confirmation_v2_files(csv_files)
    ...
    lines = [
        "# Stage 3V V2 held-out confirmation",
        "",
