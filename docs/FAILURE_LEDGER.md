...
**2026-09-02 non-actuating successor work:** the exact nominal 1.9.0 target/return trajectories and analytical IK were reconstructed offline, with a minimum 42.706° supplied configured-limit margin. This did not resolve the failure: analytical collision checking, live present/target telemetry, tracking, load/current, cable and enclosure clearance, and an actual return from a measured target remain absent. A receive-only recorder and split target/return state machine were added, but the recorder has not run, the observability patch is uninstalled, and no executor exists. Limited owner permission for the temporary observational daemon and restoration is privately recorded; motion permission and independent human approval are absent. Therefore the physical blocker and 0/4 accepted V4 directions remain unchanged.

**2026-09-04 offline failure rehearsal:** four fixed local mock-process cases—start failure, health timeout, state-stream disconnect, and shutdown hang—passed duplicate-start refusal and restoration-interlock checks. The harness confirmed mock exit before opening its mock restoration gate and authorized zero hardware actions. This does not establish that a real daemon releases the serial bus, disables torque, or can safely be killed or restarted. Unit-specific Pollen guidance and the physical blocker remain outstanding.
