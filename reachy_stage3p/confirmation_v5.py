"""Fresh held-out evaluation for the frozen Stage 3P V5 relative visual servo."""
...
from .confirmation_protocol_v5 import CONFIRMATION_V5_STEPS, SUCCESS_CRITERIA, protocol_payload
from .policy_v5 import Stage3PVisualServoPolicyV5, Stage3PVisualServoV5Spec
from .policy_v5_freeze import POLICY_PATH, TOURNAMENT_PATH, verify_policy_v5_freeze
...
def frozen_selected_spec() -> Stage3PVisualServoV5Spec:
    verified = verify_policy_v5_freeze()
    ...
    if (
        ...
        raise ValueError("The frozen selected Stage 3P V5 policy cannot be resolved uniquely.")
    ...
    if (
        ...
        raise ValueError("The V5 confirmation protocol does not match the frozen policy bundle.")
    return Stage3PVisualServoV5Spec(**{
        key: source[key] for key in Stage3PVisualServoV5Spec.__dataclass_fields__
    })
...
def evaluate_confirmation_v5_trial(
    step: Any,
    rows: list[dict[str, Any]],
    spec: Stage3PVisualServoV5Spec | None = None,
) -> dict[str, Any]:
    active_spec = spec or frozen_selected_spec()
    policy = Stage3PVisualServoPolicyV5(active_spec)
    shadow = CoupledMotionShadow(
        maximum_abs_pitch_deg=active_spec.maximum_abs_increment_deg,
    ...
    pretransition_confirmed_rows = 0
    pretransition_adjustments = 0
    eligible_adjustments: list[float] = []
    scored_adjustments: list[float] = []
    ...
    for row in rows:
        ...
        if transition_ms is not None and elapsed < transition_ms and evidence.confirmed:
            ...
            pretransition_confirmed_rows += 1
        decision = shadow.process(elapsed, evidence)
        if decision.action != "WOULD_ADJUST" or decision.target_pitch_deg is None:
            continue
        increment = float(decision.target_pitch_deg)
        all_adjustments.append(increment)
        if transition_ms is not None and elapsed < transition_ms:
            pretransition_adjustments += 1
            continue
        if transition_ms is not None and not associated_before_transition:
            continue
        eligible_adjustments.append(increment)
        if scoring_start_ms is None or elapsed >= scoring_start_ms:
            scored_adjustments.append(increment)

    target_pitch = step.target_pitch_deg
    wrong_sign = 0
    if target_pitch not in (None, 0.0):
        wrong_sign = sum(
            math.copysign(1.0, increment) != math.copysign(1.0, float(target_pitch))
            for increment in eligible_adjustments
        )
    ...
    return {
        ...
        "would_adjust_rows": len(all_adjustments),
        "eligible_adjustment_rows": len(eligible_adjustments),
        "scored_adjustment_rows": len(scored_adjustments),
        "first_target_pitch_deg": (
            scored_adjustments[0] if scored_adjustments else None
        ),
        "median_scored_increment_deg": (
            median(scored_adjustments) if scored_adjustments else None
        ),
        "maximum_abs_increment_deg": max(
            (abs(value) for value in all_adjustments), default=0.0
        ),
        ...
        "pretransition_confirmed_rows": (
            None if transition_ms is None else pretransition_confirmed_rows
        ),
...
def _role_pitch_summary(trials: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    limit = float(SUCCESS_CRITERIA["bounded_increment"]["maximum_abs_increment_deg"])
    for role in ("matching_acquisition", "maintenance_transition"):
        for pitch in (-10.0, 10.0):
            ...
            row: dict[str, Any] = {
                ...
                "pitch_deg": pitch,
                "trials": len(group),
                "trials_with_relative_correction": len(moving),
                "coverage_passed": len(moving) == 3,
                "maximum_abs_increment_deg": max(
                    (float(trial["maximum_abs_increment_deg"]) for trial in group),
                    default=0.0,
                ),
                "bounded_passed": all(
                    float(trial["maximum_abs_increment_deg"]) <= limit for trial in group
                ),
...
def aggregate_confirmation_v5_trials(trials: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    hard_negative_adjustments = sum(int(trial["would_adjust_rows"]) for trial in negatives)
    gates = {
        "hard_negative_safety": hard_negative_adjustments == 0,
        "vertical_direction": all(row["wrong_sign_adjustments"] == 0 for row in summaries),
        "bounded_increment": all(row["bounded_passed"] for row in summaries),
        "static_relative_correction_coverage": all(
            row["coverage_passed"] for row in acquisition
        ),
        "pretransition_association": all(
            row["pretransition_associations"] == 3 for row in maintenance
        ),
        "pretransition_center_hold": all(
            row["pretransition_adjustment_rows"] == 0 for row in maintenance
        ),
        "silent_maintenance_relative_correction_coverage": all(
    ...
    return {
        ...
        "hard_negative_would_adjust_rows": hard_negative_adjustments,
...
def evaluate_saved_files(csv_files: list[str]) -> dict[str, Any]:
    if len(csv_files) != len(CONFIRMATION_V5_STEPS):
        raise ValueError("A complete fresh Stage 3P V5 confirmation requires 18 accepted files.")
    spec = frozen_selected_spec()
    fingerprint = protocol_payload()["fingerprint"]
    trials: list[dict[str, Any]] = []
    for step, filename in zip(CONFIRMATION_V5_STEPS, csv_files):
        path = (DATA_DIR / filename).resolve()
        if path.parent != DATA_DIR or not path.is_file():
            raise ValueError(f"Fresh held-out Stage 3P V5 file is missing: {filename}")
        rows = load_rows(path)
        issues = quality_issues(step, summarise_rows(rows))
        if issues:
            raise ValueError(f"Held-out V5 file fails quality: {filename}: {'; '.join(issues)}")
        ...
        if not metadata_path.is_file() or not compliance_path.is_file():
            raise ValueError(f"Held-out V5 sidecars are incomplete: {filename}")
        ...
        if (
            ...
            raise ValueError(f"Held-out V5 metadata integrity failed: {filename}")
        if (
            ...
            raise ValueError(f"Held-out V5 audit compliance is incomplete: {filename}")
        trials.append({**evaluate_confirmation_v5_trial(step, rows, spec), "file": filename})

    aggregate = aggregate_confirmation_v5_trials(trials)
    freeze = verify_policy_v5_freeze()
    return {
        "schema": "reachy-stage3p-held-out-relative-servo-confirmation-result-v5",
...
def write_results(csv_files: list[str]) -> dict[str, Any]:
    ...
    lines = [
        "# Fresh held-out passive Stage 3P relative-servo confirmation V5",
        "",
        ...
        *[
        ...
        "",
        "> This result tests relative shadow corrections, not absolute one-shot pitch accuracy,",
        "> and does not authorize physical movement.",
    ]
