...
- **Successor work:** a receive-only simultaneous present/target recorder now fails closed when target fields are absent. An offline validator reproduced the exact 1.9.0 `GotoMove` law and analytical IK for all four proposed 3° outward and nominal return paths; the smallest configured-limit margin was 42.706°, but this is not collision, load, tracking, or safety validation. A pure state machine now requires new preflight and authorization records for the return rather than returning automatically. No successor command has been sent, owner scope is unconfirmed, and independent review is pending; see the [`trace status`](docs/RECEIVE_ONLY_SUCCESSOR_TRACE.md), [`trajectory review`](docs/SUCCESSOR_TRAJECTORY_REVIEW.md), and [`split authorization design`](docs/SPLIT_TARGET_RETURN_PROTOCOL.md).
...
The repository contains 120 package modules (20,659 lines), 41 test modules, seven public verification/diagnostic scripts, and the frozen numeric artifacts. Forty package files retain explicit version tags because rejected and superseded protocol generations were preserved.
...
| [`tests/`](tests) | Self-contained component and protocol tests. | 213 software tests are not 213 robot trials and do not validate hardware. |
...
1. Send the explicit [`owner-scope request`](docs/OWNER_SCOPE_REQUEST.md) and obtain an independent human robotics verdict using the [`complete review packet`](docs/INDEPENDENT_PROTOCOL_REVIEW.md), including the [`return-to-borrowed-condition protocol`](docs/RETURN_TO_BORROWED_CONDITION.md); preserve the real replies as hashed private artifacts.
2. If—and only if—the owner and reviewer approve the software change, install the observational schema patch and collect a bounded receive-only present/target trace. Do not combine that capture with motion.
